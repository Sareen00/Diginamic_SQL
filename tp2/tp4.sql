...
--l
SELECT bon.id, bon.date_cmde, article.designation, fournisseur.nom
FROM bon JOIN compo ON bon.id = compo.id_bon JOIN article ON compo.id_art = article.id JOIN fournisseur ON article.id_fou = fournisseur.id
WHERE article.designation LIKE '%boulon%';
